...
const updateUI = (pl) => {
  ...
  updateProjects(pl.accessToken).then(e => {
    if (pl.project) document.getElementById('project').value = pl.project;
  })
}
...
function saveSettings(sdpi_collection) {
  ...
  if (sdpi_collection.hasOwnProperty('key') && sdpi_collection.key != '') {
    ...
    if (sdpi_collection.key === 'accessToken') {
      const accessToken = sdpi_collection.value;
      const project = settings['project'];
      document.getElementById('tokenError').classList.remove('hidden');
      updateProjects(accessToken).then(e => {
...
async function updateProjects(accessToken) {
  try {
    if (accessToken) {
      await getProjects(accessToken).then(projectData => {
        if (!Utils.isObjectEmpty(projectData)) {
          document.getElementById('projectWrapper').classList.remove('hidden')
          document.getElementById('tokenError').classList.add('hidden')
          document.getElementById('project').innerHTML = '<option value="0"></option>'
          const selectEl = document.getElementById('project')

          for (projectNum in projectData) {
            const optionEl = document.createElement('option')
            optionEl.innerText = projectData[projectNum].name
            optionEl.value = projectData[projectNum].id.toString()
...
async function getProjects(accessToken) {
  const response = await fetch(
    `${nokoBaseUrl}/projects?per_page=100&enabled=true`, {
      method: 'GET',
      headers: {
        'X-NokoToken': accessToken
      }
    }
  )
