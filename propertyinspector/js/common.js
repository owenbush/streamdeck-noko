...
/**
 * Set the noko base url for use during API calls.
 */

let nokoBaseUrl = 'https://api.nokotime.com/v2';
...
/**
 * Format seconds into something that matches the Noko timer.
 */
Utils.formatSeconds = function (seconds) {
  if (seconds < 3600) {
    return '0:' + Utils.leadingZero(Math.round(seconds / 60));
  }
  return Math.floor(seconds / 3600) + ':' + Utils.leadingZero(Math.round((seconds % 3600) / 60));
};

/**
 * Add a leading zero if necessary.
 */
Utils.leadingZero = function (val) {
  if (val < 10) {
    return '0' + val;
  }
  return val;
 ...
 */
Utils.getProject = function (jsn) {
  return Utils.getProp(jsn, 'payload.settings.project', '');
};

/**
 * Get the accessToken from settings.
 */
Utils.getAccessToken = function (jsn) {
  return Utils.getProp(jsn, 'payload.settings.accessToken', '');
};

/**
 * Get the label from settings.
 */
Utils.getLabel = function (jsn) {
  return Utils.getProp(jsn, 'payload.settings.label', 'Set Proj');
};

/**
 * Get the state from settings.
 */
Utils.getState = function (jsn) {
  return Utils.getProp(jsn, 'payload.settings.state', 0);
};

/**
 * Get the context from settings.
 */
Utils.getContext = function (jsn) {
  return Utils.getProp(jsn, 'context', '');
};
